...
(function()
{
	...
	document.addEventListener('drop', function(e)
	{
		...
		//     posible_target = searching_target($(e.target));
         //    if(posible_target){
		//         e.target = posible_target;
         //    }
        // }
        if(id_page == 1){
            $('.add-page').parent().addClass('open')
        }
        else{
            target = e.target;
            if($('#sortable').find(target)[0]){
                posible_target = searching_target($(target));
                if(posible_target){
            ...
            if(target.getAttribute('data-draggable') == 'target') {
                try {
                    ...
                    if (type_component) {
                        ...
                        id_component_selected = $(html_node).attr('id');
                        ...
                        type_component = $(item).attr('data-type');
                        id_component_selected = $(item).attr('id');
                    }

                    e.preventDefault();
                    item = null;
                    $('.conf').hide();
                    $('#configuracion-' + type_component).show();
                    initialize_conections_configurations();
                    ionic3.create_component(id_component_selected);
...
function searching_target(origen_item){
    can_add = origen_item.attr('data-draggable') == 'target';
    if(origen_item.hasClass("dropzone")){return}
    if(!can_add){
        origen_item = origen_item.parent();
        return searching_target(origen_item)
